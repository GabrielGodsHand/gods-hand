...
interface THREE_Scene {
  add: (object: unknown) => void;
}

interface THREE_PerspectiveCamera {
  aspect: number;
  updateProjectionMatrix: () => void;
  position: { z: number };
}

interface TrackballControlsType {
  minDistance: number;
  maxDistance: number;
  rotateSpeed: number;
  zoomSpeed: number;
...
interface ThreeGlobeType {
  globeImageUrl: (url: string) => ThreeGlobeType;
  hexPolygonsData: (data: unknown[]) => ThreeGlobeType;
  hexPolygonResolution: (resolution: number) => ThreeGlobeType;
  hexPolygonMargin: (margin: number) => ThreeGlobeType;
  hexPolygonUseDots: (useDots: boolean) => ThreeGlobeType;
  hexPolygonColor: (colorFunc: (d: unknown) => string) => ThreeGlobeType;
  hexPolygonAltitude: (altitude: number) => ThreeGlobeType;
  ...
  useEffect(() => {
    ...
    const initGlobe = async () => {
      try {
        ...
        const [threeModule, threeGlobeModule, controlsModule] =
          await Promise.all([
            import("three"),
            import("three-globe"),
        ...
        if (!res.ok) {
          throw new Error(
            `Failed to load country data: ${res.status} ${res.statusText}`
          );
        }
        ...
        console.log(
          "All disaster locations:",
          disasters.map((d) => d.name)
        );
        ...
        testCountries.forEach((testCountry) => {
          ...
          countries.features.forEach((country: any, index: number) => {
            ...
            const hasMatch = Object.values(props).some(
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              (value: any) =>
                typeof value === "string" &&
                value.toLowerCase().includes(testCountry.toLowerCase())
            );
          ...
          if (locationToCountryMap[location]) {
            locationToCountryMap[location].forEach((countryVariant) => {
              ...
              if (foundCountry) {
                ...
                console.log(
                  `✓ Matched "${location}" to "${countryName}" via direct mapping`
                );
                matched = true;
              }
            });
          ...
          if (!matched) {
            ...
            countries.features.forEach((country: any) => {
              ...
              if (countryName === location) {
                ...
                console.log(
                  `✓ Exact match: "${location}" = "${country.properties.name}"`
                );
                matched = true;
              }
              // Partial match for longer location names
                ...
                location.length > 4 &&
                (countryName.includes(location) ||
                  location.includes(countryName))
              ) {
                affectedCountries.add(country.properties.name);
                console.log(
                  `✓ Partial match: "${location}" ~ "${country.properties.name}"`
                );
                matched = true;
              }
            });
        ...
        console.log(
          "Final affected countries from actual disasters:",
          Array.from(affectedCountries)
        );
        ...
        globe = new ThreeGlobe()
          ...
          .hexPolygonColor((d: any) => {
            ...
            if (Math.random() < 0.02) {
              // Log ~2% of countries instead of 10%
              console.log(
                `Coloring country: "${countryName}" -> ${
                  isAffected ? "RED (#dc2626)" : "GOLDEN (#f7d794)"
                }`
              );
            }
              ...
              if (object.userData && object.userData.feature) {
                ...
                if (affectedCountries.has(countryName)) {
                  ...
                  const countryDisasters = disasters.filter((disaster) => {
                    ...
                    if (locationToCountryMap[location]) {
                      return locationToCountryMap[location].some(
                        (variant) =>
                    ...
                    return (
                      countryName.toLowerCase().includes(location) ||
                  ...
                  if (countryDisasters.length > 0) {
                    ...
                    console.log(
                      `Clicked on ${countryName}, found ${countryDisasters.length} disasters`
                    );
                    break;
                  }
                }
    ...
    return (
      <div className="w-full h-[70vh] flex items-center justify-center">
        <div className="text-center bg-white/10 backdrop-blur-xl rounded-3xl border border-white/20 p-12 shadow-2xl">
          ...
          <p className="text-gray-900 font-['Cinzel'] text-xl mb-2">
            Failed to load interactive globe
          </p>
      ...
      {selectedCountry && (
        ...
        >
          <div className="flex justify-between items-start mb-4">
            <h3 className="text-lg font-bold text-gray-900 font-['Cinzel']">
                ...
                <TiltedCard
                  ...
                  overlayContent={
                    <div className="w-full h-full bg-gradient-to-br from-amber-100/90 to-amber-200/90 backdrop-blur-sm rounded-[15px] p-4 border border-amber-300/50">
                      <h4 className="font-bold text-gray-900 font-['Cinzel'] mb-2 text-sm">
                        {disaster.name}
                      </h4>
                      <div className="text-xs text-gray-800 font-['Cinzel'] space-y-1 mb-3">
                        ...
                        <p>
                          ...
                          {disaster.amount.toLocaleString()}
                        </p>
                      </div>
                      {onDisasterClick && (
                        <button
