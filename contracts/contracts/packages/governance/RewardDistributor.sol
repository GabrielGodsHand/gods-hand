...
contract RewardDistributor is IRewardDistributor, Ownable {
    ...
    function claim(
        address _to
    ) external override(IRewardDistributor) returns (uint256) {
        require(
            msg.sender == canonicalRollup(),
            Errors.RewardDistributor__InvalidCaller(
                msg.sender,
                canonicalRollup()
            )
        );
    ...
    function claimBlockRewards(
        ...
        uint256 bal = ASSET.balanceOf(address(this));
        uint256 reward = bal > BLOCK_REWARD * _blocks
            ? BLOCK_REWARD * _blocks
            : bal;
