...
import {ICoinIssuer} from "./interfaces/ICoinIssuer.sol";
...
contract CoinIssuer is ICoinIssuer, Ownable {
    ...
    function mint(
        address _to,
        uint256 _amount
    ) external override(ICoinIssuer) onlyOwner {
        uint256 maxMint = mintAvailable();
        require(
            _amount <= maxMint,
            Errors.CoinIssuer__InsufficientMintAvailable(maxMint, _amount)
        );
        timeOfLastMint = block.timestamp;
