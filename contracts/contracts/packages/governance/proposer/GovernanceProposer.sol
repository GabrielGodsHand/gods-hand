...
import {GSEPayload} from "../GSEPayload.sol";
import {IEmpire} from "../interfaces/IEmpire.sol";
import {IGovernance} from "../interfaces/IGovernance.sol";
import {IGovernanceProposer} from "../interfaces/IGovernanceProposer.sol";
...
contract GovernanceProposer is IGovernanceProposer, EmpireBase {
    ...
    function getProposalProposer(
        uint256 _proposalId
    ) external view override(IGovernanceProposer) returns (address) {
    ...
    function _execute(
        ...
        uint256 proposalId = IGovernance(getExecutor()).propose(
            IPayload(address(extendedPayload))
        );
