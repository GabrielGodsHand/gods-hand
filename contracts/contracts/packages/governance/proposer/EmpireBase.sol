...
import {SignatureLib, Signature} from "../../shared/libraries/SignatureLib.sol";
import {IEmpire, IEmperor} from "../interfaces/IEmpire.sol";
import {Slot} from "../../shared/libraries/TimeMath.sol";
...
abstract contract EmpireBase is EIP712, IEmpire {
    ...
    bytes32 public constant VOTE_TYPEHASH =
        keccak256("Vote(address proposal,uint256 nonce)");
    ...
    mapping(address instance => mapping(uint256 roundNumber => RoundAccounting))
        public rounds;
    ...
    function vote(
        IPayload _proposal
    ) external override(IEmpire) returns (bool) {
        return
            _internalVote(
                _proposal,
                Signature({v: 0, r: bytes32(0), s: bytes32(0)})
            );
    }
    ...
    function executeProposal(
        ...
        require(
            ...
            Errors.GovernanceProposer__ProposalTooOld(
                _roundNumber,
                currentRound
            )
        );
        ...
        require(
            !round.executed,
            Errors.GovernanceProposer__ProposalAlreadyExecuted(_roundNumber)
        );
        require(
            round.leader != IPayload(address(0)),
            Errors.GovernanceProposer__ProposalCannotBeAddressZero()
        );
        uint256 votesCast = round.yeaCount[round.leader];
        require(
            votesCast >= N,
            Errors.GovernanceProposer__InsufficientVotes(votesCast, N)
        );
    ...
    function yeaCount(
        ...
        uint256 _round,
        IPayload _proposal
    ) external view override(IEmpire) returns (uint256) {
    ...
    function computeRound(
        Slot _slot
    ) public view override(IEmpire) returns (uint256) {
        return Slot.unwrap(_slot) / M;
    }

    function getVoteSignatureDigest(
        IPayload _proposal,
        address _proposer
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(VOTE_TYPEHASH, _proposal, nonces[_proposer])
                )
            );
    }
    ...
    function _internalVote(
        ...
        if (_sig.isEmpty()) {
            require(
                msg.sender == proposer,
                Errors.GovernanceProposer__OnlyProposerCanVote(
                    msg.sender,
                    proposer
                )
            );
        ...
        if (
            ...
            round.yeaCount[_proposal] > round.yeaCount[round.leader]
        ) {
            round.leader = _proposal;
        }
