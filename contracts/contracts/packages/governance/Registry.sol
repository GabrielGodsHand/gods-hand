...
import {IHaveVersion, IRegistry} from "./interfaces/IRegistry.sol";
...
contract Registry is IRegistry, Ownable {
     ...
     */
    function getCanonicalRollup()
        external
        view
        override(IRegistry)
        returns (IHaveVersion)
    {
    ...
    function getRollup(
        uint256 _version
    ) external view override(IRegistry) returns (IHaveVersion) {
        IHaveVersion rollup = $.versionToRollup[_version];
        require(
            address(rollup) != address(0),
            Errors.Registry__RollupNotRegistered(_version)
        );
        return rollup;
    }

    function numberOfVersions()
        external
        view
        override(IRegistry)
        returns (uint256)
    ...
    function getVersion(
        uint256 _index
    ) external view override(IRegistry) returns (uint256) {
