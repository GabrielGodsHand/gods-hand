...
import {IGovernance, Proposal, ProposalState, ProposeConfiguration, Configuration, Ballot, Withdrawal} from "./interfaces/IGovernance.sol";
import {IPayload} from "./interfaces/IPayload.sol";
import {ConfigurationLib} from "./libraries/ConfigurationLib.sol";
import {Errors} from "./libraries/Errors.sol";
import {ProposalLib, VoteTabulationReturn} from "./libraries/ProposalLib.sol";
import {User, UserLib} from "./libraries/UserLib.sol";
...
contract Governance is IGovernance {
    ...
    mapping(uint256 proposalId => mapping(address user => Ballot ballot))
        public ballots;
    ...
    modifier onlySelf() {
        require(
            ...
            Errors.Governance__CallerNotSelf(msg.sender, address(this))
        );
        _;
    }
    ...
    modifier isDepositAllowed(address _caller) {
        require(
            depositControl.allDepositsAllowed ||
                depositControl.isAllowed[_caller],
    ...
    function addDepositor(
        address _depositor
    ) external override(IGovernance) onlySelf {
        depositControl.isAllowed[_depositor] = true;
        emit DepositorAdded(_depositor);
    }
    ...
    function updateGovernanceProposer(
        address _governanceProposer
    ) external override(IGovernance) onlySelf {
        governanceProposer = _governanceProposer;
    ...
    function deposit(
        address _onBehalfOf,
        uint256 _amount
    ) external override(IGovernance) isDepositAllowed(_onBehalfOf) {
    ...
    function initiateWithdraw(
        address _to,
        uint256 _amount
    ) external override(IGovernance) returns (uint256) {
    ...
    function vote(
        ...
        uint256 _amount,
        bool _support
    ) external override(IGovernance) returns (bool) {
        ProposalState state = getProposalState(_proposalId);
        require(
            state == ProposalState.Active,
            Errors.Governance__ProposalNotActive()
        );

        // Compute the power at the time where we became active
        uint256 userPower = users[msg.sender].powerAt(
            proposals[_proposalId].pendingThrough()
        );
    ...
    function execute(
        ...
        require(
            ...
            Errors.Governance__ProposalNotExecutable()
        );
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            require(
                actions[i].target != address(ASSET),
                Errors.Governance__CannotCallAsset()
            );
    ...
    function powerAt(
        address _owner,
        Timestamp _ts
    ) external view override(IGovernance) returns (uint256) {
    ...
    function isAllowedToDeposit(
        address _caller
    ) external view override(IGovernance) returns (bool) {
        return depositControl.isAllowed[_caller];
    }

    function isAllDepositsAllowed()
        external
        view
    ...
    function getProposal(
        uint256 _proposalId
    ) external view override(IGovernance) returns (Proposal memory) {
        return proposals[_proposalId];
    }

    function getWithdrawal(
        uint256 _withdrawalId
    ) external view override(IGovernance) returns (Withdrawal memory) {
    ...
    function getProposalState(
        uint256 _proposalId
    ) public view override(IGovernance) returns (ProposalState) {
        require(
            _proposalId < proposalCount,
            Errors.Governance__ProposalDoesNotExists(_proposalId)
        );
        ...
        if (
            governanceProposer != self.proposer &&
            address(this) != self.proposer
        ) {
