...
import {Proposal} from "./interfaces/IGovernance.sol";
import {IPayload} from "./interfaces/IPayload.sol";
import {AddressSnapshotLib, SnapshottedAddressSet} from "./libraries/AddressSnapshotLib.sol";
import {DelegationLib, DelegationData} from "./libraries/DelegationLib.sol";
...
interface IGSE is IGSECore {
    ...
    function getWithdrawer(
        address _instance,
        address _attester
    ) external view returns (address, bool, address);
    function balanceOf(
        address _instance,
        address _attester
    ) external view returns (uint256);
    function effectiveBalanceOf(
        address _instance,
        address _attester
    ) external view returns (uint256);
    ...
    function getConfig(
        address _instance,
        address _attester
    ) external view returns (AttesterConfig memory);
    function getAttesterCountAtTime(
        address _instance,
        Timestamp _timestamp
    ) external view returns (uint256);
    ...
    function getAttestersAtTime(
        address _instance,
        Timestamp _timestamp
    ) external view returns (address[] memory);
    function getAttesterFromIndexAtTime(
        address _instance,
        uint256 _index,
        Timestamp _timestamp
    ) external view returns (address);
...
contract GSECore is IGSECore, Ownable {
    ...
    mapping(address instanceAddress => InstanceStaking instance)
        internal instances;
    ...
    modifier onlyRollup() {
        require(
            isRollupRegistered(msg.sender),
            Errors.GSE__NotRollup(msg.sender)
        );
        _;
    }
    ...
    function addRollup(address _rollup) external override(IGSECore) onlyOwner {
        require(
            _rollup != address(0),
            Errors.GSE__InvalidRollupAddress(_rollup)
        );
        require(
            !instances[_rollup].exists,
            Errors.GSE__RollupAlreadyRegistered(_rollup)
        );
    ...
    function deposit(
        ...
        require(
            !_onCanonical || isCanonical,
            Errors.GSE__NotCanonical(instanceAddress)
        );
        ...
        instances[instanceAddress].configOf[_attester] = AttesterConfig({
    ...
    function withdraw(
        address _attester,
        uint256 _amount
    ) external override(IGSECore) onlyRollup returns (uint256, bool, uint256) {
        address instanceAddress = msg.sender;
        ...
        require(
            balance >= _amount,
            Errors.GSE__InsufficientStake(balance, _amount)
        );
        ...
        if (isRemoved) {
            require(
                instanceStaking.attesters.remove(_attester),
                Errors.GSE__FailedToRemove(_attester)
            );
        ...
        uint256 withdrawalId = getGovernance().initiateWithdraw(
            msg.sender,
            amountWithdrawn
        );
    ...
    function proposeWithLock(
        IPayload _proposal,
        address _to
    ) external override(IGSECore) returns (uint256) {
        Governance gov = getGovernance();
    ...
    function delegate(
        ...
        address _attester,
        address _delegatee
    ) external override(IGSECore) {
        require(
            isRollupRegistered(_instance),
            Errors.GSE__InstanceDoesNotExist(_instance)
        );
        address withdrawer = instances[_instance]
            .configOf[_attester]
            .withdrawer;
        require(
    ...
    function voteWithCanonical(
        ...
        require(
            msg.sender == getCanonicalAt(ts),
            Errors.GSE__NotCanonical(msg.sender)
        );
        _vote(CANONICAL_MAGIC_ADDRESS, _proposalId, _amount, _support);
    }

    function isRollupRegistered(
        address _instance
    ) public view override(IGSECore) returns (bool) {
    ...
    function getCanonicalAt(
        Timestamp _timestamp
    ) public view override(IGSECore) returns (address) {
        return
            address(
                canonical
                    .upperLookup(Timestamp.unwrap(_timestamp).toUint32())
                    .toUint160()
            );
    }
...
contract GSE is IGSE, GSECore {
    ...
    ) GSECore(__owner, _stakingAsset) {}

    function getConfig(
        address _instance,
        address _attester
    ) external view override(IGSE) returns (AttesterConfig memory) {
        (
        ...
        )
    {
        ...
        return (
            instanceStaking.configOf[_attester].withdrawer,
            true,
            instanceAddress
        );
    }
    ...
    function balanceOf(
        address _instance,
        address _attester
    ) external view override(IGSE) returns (uint256) {
    ...
    function effectiveBalanceOf(
        ...
        if (getCanonical() == _instance) {
            balance += delegation.getBalanceOf(
                CANONICAL_MAGIC_ADDRESS,
                _attester
            );
        }
        return balance;
    ...
    function getDelegatee(
        address _instance,
        address _attester
    ) external view override(IGSE) returns (address) {
    ...
    function getAttestersAtTime(
        ...
        return
            _getAddressFromIndicesAtTimestamp(_instance, indices, _timestamp);
    }
    ...
    function getAttestersFromIndicesAtTime(
        ...
        return
            _getAddressFromIndicesAtTimestamp(_instance, _indices, _timestamp);
    }
    ...
    function getCanonicalMagicAddress()
        external
        pure
        override(IGSE)
        returns (address)
    ...
    function getVotingPowerAt(
        address _delegatee,
        Timestamp _timestamp
    ) public view override(IGSE) returns (uint256) {
    ...
    function getAttesterCountAtTime(
        ...
        if (getCanonicalAt(_timestamp) == _instance) {
            count += instances[CANONICAL_MAGIC_ADDRESS]
                .attesters
                .lengthAtTimestamp(timestamp);
        }
    ...
    function _getAddressFromIndicesAtTimestamp(
        ...
        InstanceStaking storage store = instances[_instance];
        InstanceStaking storage canonicalStore = instances[
            CANONICAL_MAGIC_ADDRESS
        ];
        ...
        uint256 canonicalSize = isCanonical
            ? canonicalStore.attesters.lengthAtTimestamp(ts)
            : 0;
        ...
        for (uint256 i = 0; i < _indices.length; i++) {
            ...
            require(
                index < totalSize,
                Errors.GSE__OutOfBounds(index, totalSize)
            );

            if (index < storeSize) {
                attesters[i] = store.attesters.getAddressFromIndexAtTimestamp(
                    index,
                    ts
                );
            } else if (isCanonical) {
                attesters[i] = canonicalStore
                    .attesters
                    .getAddressFromIndexAtTimestamp(index - storeSize, ts);
            } else {
    ...
    function _getInstanceStoreWithAttester(
        address _instance,
        address _attester
    ) internal view returns (InstanceStaking storage, bool, address) {
        InstanceStaking storage store = instances[_instance];
