...
import {IRegistry} from "../governance/interfaces/IRegistry.sol";
...
contract StakingAssetHandler is IStakingAssetHandler, Ownable {
    ...
    function dripQueue() external override(IStakingAssetHandler) {
        ...
        uint256 depositsToMake = (queueLength < mintsAvailable)
            ? queueLength
            : mintsAvailable;
    ...
    function setMintInterval(
        uint256 _interval
    ) external override(IStakingAssetHandler) onlyOwner {
        mintInterval = _interval;
        emit IntervalUpdated(_interval);
    }

    function setDepositsPerMint(
        uint256 _depositsPerMint
    ) external override(IStakingAssetHandler) onlyOwner {
        require(_depositsPerMint > 0, CannotMintZeroAmount());
        depositsPerMint = _depositsPerMint;
    ...
    function setZKPassportVerifier(
    ...
    function _validatePassportProof(
        ...
        (bool verified, bytes32 nullifier) = zkPassportVerifier.verifyProof(
            _params
        );
        ...
        require(
            zkPassportVerifier.verifyScopes(
                _params.publicInputs,
                validScope,
                validSubscope
            ),
            InvalidScope()
        ...
        if (!skipBindCheck) {
            ...
            require(
                boundAddress == _attester,
                ProofNotBoundToAddress(boundAddress, _attester)
            );
        }
