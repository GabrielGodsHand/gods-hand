...
import {IStakingCore} from "../core/interfaces/IStaking.sol";
...
contract SlashPayload is IPayload {
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _validators.length; i++) {
            offenders.push(
                Offender({validator: _validators[i], amount: _amounts[i]})
            );
        }
        ...
        for (uint256 i = 0; i < offenders.length; i++) {
            actions[i] = IPayload.Action({
                ...
                data: abi.encodeWithSelector(
                    IStakingCore.slash.selector,
                    offenders[i].validator,
                    offenders[i].amount
                )
            });
        }
