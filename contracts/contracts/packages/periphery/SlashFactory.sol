...
contract SlashFactory is ISlashFactory {
    ...
    function createSlashPayload(
        ...
        require(
            ...
            ISlashFactory.SlashPayloadAmountsLengthMismatch(
                _validators.length,
                _amounts.length
            )
        );
        require(
            ...
            ISlashFactory.SlashPayloadOffensesLengthMismatch(
                _validators.length,
                _offenses.length
            )
        );
        ...
        SlashPayload payload = new SlashPayload{salt: salt}(
            _validators,
            _amounts,
            VALIDATOR_SELECTION
        );

        emit SlashPayloadCreated(
            address(payload),
            _validators,
            _amounts,
            _offenses
    ...
    function getAddressAndIsDeployed(
        ...
        (address predictedAddress, bytes32 salt) = _computeSlashPayloadAddress(
            _validators,
            _amounts
        );
    ...
    function _computeSlashPayloadAddress(
        ...
        bytes32 creationCodeHash = keccak256(
            abi.encodePacked(type(SlashPayload).creationCode, constructorArgs)
        );
