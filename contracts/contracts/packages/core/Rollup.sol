...
import {IRollup, IHaveVersion, ChainTips, PublicInputArgs, L1FeeData, ManaBaseFeeComponents, FeeAssetPerEthE9, BlockLog, BlockHeaderValidationFlags, FeeHeader, RollupConfigInput} from "./interfaces/IRollup.sol";
import {IStaking, AttesterConfig, Exit, AttesterView, Status} from "./interfaces/IStaking.sol";
import {IValidatorSelection, IEmperor} from "./interfaces/IValidatorSelection.sol";
import {IVerifier} from "./interfaces/IVerifier.sol";
import {FeeLib, FeeHeaderLib, FeeAssetValue, PriceLib} from "./libraries/rollup/FeeLib.sol";
import {ProposedHeader} from "./libraries/rollup/ProposedHeaderLib.sol";

import {StakingLib} from "./libraries/rollup/StakingLib.sol";
...
import {RollupCore, GenesisState, IRewardDistributor, IFeeJuicePortal, IERC20, TimeLib, Slot, Epoch, Timestamp, Errors, CommitteeAttestation, ExtRollupLib, ExtRollupLib2, EthValue, STFLib, RollupStore, IInbox, IOutbox} from "./RollupCore.sol";
...
contract Rollup is IStaking, IValidatorSelection, IRollup, RollupCore {
    ...
    function getCommitteeAt(
        Timestamp _ts
    ) external override(IValidatorSelection) returns (address[] memory) {
     ...
     */
    function getCommitteeCommitmentAt(
        Timestamp _ts
    ) external override(IValidatorSelection) returns (bytes32, uint256) {
    ...
    function canProposeAtTime(
        ...
        require(
            tipArchive == _archive,
            Errors.Rollup__InvalidArchive(tipArchive, _archive)
        );
    ...
    function getTargetCommitteeSize()
        external
        view
        override(IValidatorSelection)
        returns (uint256)
    ...
    function getProofSubmissionEpochs()
        external
        view
        override(IRollup)
        returns (uint256)
    ...
    function getEpochProofPublicInputs(
        ...
        return
            ExtRollupLib.getEpochProofPublicInputs(
                _start,
                _end,
                _args,
                _fees,
    ...
    function getPendingBlockNumber()
        external
        view
        override(IRollup)
        returns (uint256)
    ...
    ) external view override(IRollup) returns (BlockLog memory) {
        RollupStore storage rollupStore = STFLib.getStorage();
        require(
    ...
    function getFeeHeader(
        uint256 _blockNumber
    ) external view override(IRollup) returns (FeeHeader memory) {
        return
            FeeHeaderLib.decompress(
                FeeLib.getStorage().feeHeaders[_blockNumber]
            );
    }

    function getBlobCommitmentsHash(
        uint256 _blockNumber
    ) external view override(IRollup) returns (bytes32) {
        ...
        returns (bytes32)
    {
        ...
        return
            rollupStore
                .blocks[rollupStore.tips.pendingBlockNumber]
                .blobCommitmentsHash;
    }

    function getConfig(
        address _attester
    ) external view override(IStaking) returns (AttesterConfig memory) {
        return StakingLib.getConfig(_attester);
    }

    function getExit(
        address _attester
    ) external view override(IStaking) returns (Exit memory) {
        return StakingLib.getExit(_attester);
    }

    function getStatus(
        address _attester
    ) external view override(IStaking) returns (Status) {
        return StakingLib.getStatus(_attester);
    }

    function getAttesterView(
        address _attester
    ) external view override(IStaking) returns (AttesterView memory) {
        return StakingLib.getAttesterView(_attester);
    }

    function getActivityScore(
        address _prover
    ) external view override(IRollup) returns (ActivityScore memory) {
        return RewardLib.getActivityScore(_prover);
    }

    function getSharesFor(
        address _prover
    ) external view override(IRollup) returns (uint256) {
     ...
     */
    function getSampleSeedAt(
        Timestamp _ts
    ) external view override(IValidatorSelection) returns (uint256) {
    ...
    function getTimestampForSlot(
        Slot _slotNumber
    ) external view override(IValidatorSelection) returns (Timestamp) {
    ...
    function getSlotAt(
        Timestamp _ts
    ) external view override(IValidatorSelection) returns (Slot) {
     ...
     */
    function getEpochAtSlot(
        Slot _slotNumber
    ) external view override(IValidatorSelection) returns (Epoch) {
    ...
    function getHasSubmitted(
        ...
        uint256 _length,
        address _prover
    ) external view override(IRollup) returns (bool) {
    ...
    function getProvingCostPerManaInEth()
        external
        view
        override(IRollup)
        returns (EthValue)
    ...
    {
        return
            FeeLib.getStorage().provingCostPerMana.toFeeAsset(
                getFeeAssetPerEth()
            );
    }
    ...
    ) external view override(IRollup) returns (L1FeeData memory) {
        return FeeLib.getL1FeesAt(_timestamp);
    }

    function canPruneAtTime(
        Timestamp _ts
    ) external view override(IRollup) returns (bool) {
    ...
    function getEpochCommittee(
        Epoch _epoch
    ) public override(IValidatorSelection) returns (address[] memory) {
    ...
    function getProposerAt(
        Timestamp _ts
    ) public override(IValidatorSelection) returns (address) {
     ...
     */
    function getAttesterAtIndex(
        uint256 _index
    ) public view override(IStaking) returns (address) {
    ...
    function getManaBaseFeeAt(
        Timestamp _timestamp,
        bool _inFeeAsset
    ) public view override(IRollup) returns (uint256) {
        return
            FeeLib.summedBaseFee(
                getManaBaseFeeComponentsAt(_timestamp, _inFeeAsset)
            );
    }
    ...
    function getManaBaseFeeComponentsAt(
        Timestamp _timestamp,
        bool _inFeeAsset
    ) public view override(IRollup) returns (ManaBaseFeeComponents memory) {
    ...
    function getFeeAssetPerEth()
        ...
        view
        override(IRollup)
        returns (FeeAssetPerEthE9)
    {
        return
            FeeLib.getFeeAssetPerEthAtBlock(
                STFLib.getStorage().tips.pendingBlockNumber
            );
    }

    function getEpochForBlock(
        uint256 _blockNumber
    ) public view override(IRollup) returns (Epoch) {
    ...
    function archiveAt(
        uint256 _blockNumber
    ) public view override(IRollup) returns (bytes32) {
        RollupStore storage rollupStore = STFLib.getStorage();
        return
            _blockNumber <= rollupStore.tips.pendingBlockNumber
    ...
    function getEpochAt(
        Timestamp _ts
    ) public view override(IValidatorSelection) returns (Epoch) {
    ...
    function getNextFlushableEpoch()
        public
        view
        override(IStaking)
        returns (Epoch)
