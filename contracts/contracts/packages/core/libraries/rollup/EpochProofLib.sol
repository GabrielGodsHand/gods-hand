...
import {SubmitEpochRootProofArgs, PublicInputArgs, IRollupCore, RollupStore} from "../../interfaces/IRollup.sol";
import {Constants} from "../ConstantsGen.sol";
import {Errors} from "../Errors.sol";
import {BlobLib} from "./BlobLib.sol";
import {CompressedFeeHeader, FeeHeaderLib} from "./FeeLib.sol";
...
library EpochProofLib {
    ...
    function submitEpochRootProof(
        ...
        rollupStore.tips.provenBlockNumber = Math.max(
    ...
    function getEpochProofPublicInputs(
...

        {
            ...
            {
                bytes32 expectedPreviousArchive = rollupStore
                    .blocks[_start - 1]
                    .archive;
                require(
                    expectedPreviousArchive == _args.previousArchive,
                    Errors.Rollup__InvalidPreviousArchive(
                        expectedPreviousArchive,
                        _args.previousArchive
                    )
                );
            }
        ...
        (
            publicInputs[offset],
            publicInputs[offset + 1],
            publicInputs[offset + 2]
        ...
        publicInputs[offset] = bytes32(
            uint256(uint248(bytes31((_blobPublicInputs[96:127]))))
        );
        // c[1]
        publicInputs[offset + 1] = bytes32(
            uint256(uint136(bytes17((_blobPublicInputs[127:144]))))
        );
        offset += 2;
    ...
    function assertAcceptable(
        ...
        require(
            startEpoch == endEpoch,
            Errors.Rollup__StartAndEndNotSameEpoch(startEpoch, endEpoch)
        );
        ...
        require(
            startEpoch > Epoch.wrap(0) || _start == 1,
            Errors.Rollup__InvalidFirstEpochProof()
        );

        bool isStartOfEpoch = _start == 1 ||
            parentEpoch <= startEpoch - Epoch.wrap(1);
        require(isStartOfEpoch, Errors.Rollup__StartIsNotFirstBlockOfEpoch());

        bool isStartBuildingOnProven = _start - 1 <=
            rollupStore.tips.provenBlockNumber;
        require(
            isStartBuildingOnProven,
            Errors.Rollup__StartIsNotBuildingOnProven()
        );

        bool claimedNumBlocksInEpoch = _end - _start + 1 <=
            Constants.AZTEC_MAX_EPOCH_DURATION;
        require(
            claimedNumBlocksInEpoch,
            Errors.Rollup__TooManyBlocksInEpoch(
                Constants.AZTEC_MAX_EPOCH_DURATION,
                _end - _start
            )
        );
    ...
    function verifyEpochRootProof(
        ...
        require(
            rollupStore.config.epochProofVerifier.verify(
                _args.proof,
                publicInputs
            ),
