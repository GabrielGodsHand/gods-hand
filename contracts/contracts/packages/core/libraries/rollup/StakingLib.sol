...
import {StakingQueueLib, StakingQueue, DepositArgs} from "../StakingQueue.sol";
import {TimeLib, Timestamp, Epoch} from "../TimeLib.sol";
import {Governance} from "../../../governance/Governance.sol";
import {GSE, AttesterConfig} from "../../../governance/GSE.sol";
...
library StakingLib {
    ...
    function vote(uint256 _proposalId) internal {
        ...
        GovernanceProposer govProposer = GovernanceProposer(
            gov.governanceProposer()
        );
        require(
            address(this) == govProposer.getInstance(),
        ...
        require(
            ...
            Errors.Staking__NotOurProposal(
                _proposalId,
                address(this),
                proposalProposer
            )
        ...
        require(
            proposal.proposer == address(govProposer),
            Errors.Staking__IncorrectGovProposer(_proposalId)
        );
    ...
    function finaliseWithdraw(address _attester) internal {
        ...
        emit IStakingCore.WithdrawFinalised(
            _attester,
            exit.recipientOrWithdrawer,
            exit.amount
        );
    }
    ...
    function slash(address _attester, uint256 _amount) internal {
        ...
        if (exit.exists) {
            ...
            (address withdrawer, bool attesterExists, ) = store
                .gse
                .getWithdrawer(address(this), _attester);
            require(attesterExists, Errors.Staking__NoOneToSlash(_attester));

            // Get the effective balance of the attester
            uint256 effectiveBalance = store.gse.effectiveBalanceOf(
                address(this),
                _attester
            );
    ...
    function deposit(
        ...
        require(
            !store.exits[_attester].exists,
            Errors.Staking__AlreadyExiting(_attester)
        );
    ...
    function flushEntryQueue(uint256 _maxAddableValidators) internal {
        Epoch currentEpoch = TimeLib.epochFromTimestamp(
            Timestamp.wrap(block.timestamp)
        );
        StakingStorage storage store = getStorage();
        require(
            store.nextFlushableEpoch <= currentEpoch,
            Errors.Staking__QueueAlreadyFlushed(currentEpoch)
        );
    ...
    function initiateWithdraw(
        ...
        require(
            ...
            Errors.Staking__InvalidRecipient(_recipient)
        );
        StakingStorage storage store = getStorage();
        ...
        if (store.exits[_attester].exists) {
            ...
            require(
                !store.exits[_attester].isRecipient,
                Errors.Staking__NothingToExit(_attester)
            );
            require(
                store.exits[_attester].recipientOrWithdrawer == msg.sender,
                Errors.Staking__NotWithdrawer(
                    store.exits[_attester].recipientOrWithdrawer,
                    msg.sender
                )
            );
            ...
            require(
                ...
                Errors.Staking__NotWithdrawer(withdrawer, msg.sender)
            );

            uint256 amount = store.gse.effectiveBalanceOf(
                address(this),
                _attester
            );
            (uint256 actualAmount, bool removed, uint256 withdrawalId) = store
                .gse
                .withdraw(_attester, amount);
    ...
    function isSlashable(address _attester) internal view returns (bool) {
        ...
        (, bool attesterExists, ) = store.gse.getWithdrawer(
            address(this),
            _attester
        );
    ...
    function getAttesterCountAtTime(
        ...
        return
            getStorage().gse.getAttesterCountAtTime(address(this), _timestamp);
    }
    ...
    function getAttestersFromIndicesAtTime(
        ...
        return
            getStorage().gse.getAttestersFromIndicesAtTime(
                address(this),
                _timestamp,
                _indices
            );
    ...
    function getConfig(
        address _attester
    ) internal view returns (AttesterConfig memory) {
        return getStorage().gse.getConfig(address(this), _attester);
    }

    function getAttesterView(
        address _attester
    ) internal view returns (AttesterView memory) {
        return
            AttesterView({
                status: getStatus(_attester),
                effectiveBalance: getStorage().gse.effectiveBalanceOf(
                    address(this),
                    _attester
                ),
    ...
    function getStatus(address _attester) internal view returns (Status) {
        ...
        uint256 effectiveBalance = getStorage().gse.effectiveBalanceOf(
            address(this),
            _attester
        );
