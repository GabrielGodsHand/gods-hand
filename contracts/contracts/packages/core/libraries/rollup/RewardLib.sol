...
import {RollupStore, SubmitEpochRootProofArgs} from "../../interfaces/IRollup.sol";
...
library ActivityScoreLib {
    ...
    function markActive(
        ActivityScore storage _score,
        RewardConfig storage _config
        ...
        _score.value = Math
            .min(curr.value + _config.increment, _config.maxScore)
            .toUint32();
    ...
    function current(
        ActivityScore storage _score
    ) internal view returns (ActivityScore memory) {
        Epoch currentEpoch = TimeLib.epochFromTimestamp(
            Timestamp.wrap(block.timestamp)
        );
        uint256 decrease = (Epoch.unwrap(currentEpoch) -
            Epoch.unwrap(_score.time)) * 1e5;
        return
            ActivityScore({
                value: decrease > uint256(_score.value)
                    ? 0
                    : _score.value - decrease.toUint32(),
                time: currentEpoch
            });
    }
...
library RewardLib {
    ...
    bytes32 private constant REWARD_STORAGE_POSITION =
        keccak256("aztec.reward.storage");
    ...
    function claimProverRewards(
        ...
        for (uint256 i = 0; i < _epochs.length; i++) {
            require(
                ...
                Errors.Rollup__NotPastDeadline(
                    _epochs[i].toDeadlineEpoch(),
                    currentEpoch
                )
            );

            require(
                !rewardStorage.proverClaimed[msg.sender].get(
                    Epoch.unwrap(_epochs[i])
                ),
                Errors.Rollup__AlreadyClaimed(msg.sender, _epochs[i])
            );
            rewardStorage.proverClaimed[msg.sender].set(
                Epoch.unwrap(_epochs[i])
            );
    ...
    function handleRewardsAndFees(
        SubmitEpochRootProofArgs memory _args,
        Epoch _endEpoch
    ) internal {
        RollupStore storage rollupStore = STFLib.getStorage();
        RewardStorage storage rewardStorage = getStorage();

        bool isRewardDistributorCanonical = address(this) ==
            rollupStore.config.rewardDistributor.canonicalRollup();
...

        {
            ...
            uint256 shares = ActivityScoreLib.toShares(
                rewardStorage.activityScores[prover].markActive(
                    rewardStorage.config
                ),
                rewardStorage.config
            );
        ...
        if (length > $er.longestProvenLength) {
...

            {
                ...
                uint256 blockRewardsAvailable = isRewardDistributorCanonical
                    ? rollupStore.config.rewardDistributor.claimBlockRewards(
                        address(this),
                        added
                    )
                    : 0;
                uint256 sequencerShare = BpsLib.mul(
                    blockRewardsAvailable,
                    rewardStorage.config.sequencerBps
                );
            ...
            for (uint256 i = $er.longestProvenLength; i < length; i++) {
                CompressedFeeHeader storage feeHeader = feeStore.feeHeaders[
                    _args.start + i
                ];
                ...
                v.proverFee = Math.min(
                    v.manaUsed * feeHeader.getProverCost(),
                    fee - burn
                );
...

                {
                    ...
                    rewardStorage.sequencerRewards[v.sequencer] += (v
                        .sequencerBlockReward + v.sequencerFee);
                }
            }
            ...
            if (t.feesToClaim > 0) {
                rollupStore.config.feeAssetPortal.distributeFees(
                    address(this),
                    t.feesToClaim
                );
            }
    ...
    function toShares(address _prover) internal view returns (uint256) {
        return
            getStorage().activityScores[_prover].toShares(getStorage().config);
    }
    ...
    function getHasSubmitted(
        ...
        return
            getStorage().epochRewards[_epoch].subEpoch[_length].shares[
                _prover
            ] > 0;
    }
    ...
    function getStorage()
        internal
        pure
        returns (RewardStorage storage storageStruct)
    {
