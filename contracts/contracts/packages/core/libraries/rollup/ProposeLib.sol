...
import {RollupStore, IRollupCore, BlockLog, BlockHeaderValidationFlags} from "../../interfaces/IRollup.sol";
...
import {SignatureDomainSeparator, CommitteeAttestation} from "../../../shared/libraries/SignatureLib.sol";
...
library ProposeLib {
    ...
    function propose(
        ...
        (v.blobHashes, v.blobsHashesCommitment, v.blobCommitments) = BlobLib
            .validateBlobs(_blobsInput, _checkBlob);
        ...
        bool isFirstBlockOfEpoch = currentEpoch >
            STFLib.getEpochForBlock(blockNumber - 1) ||
            blockNumber == 1;
    ...
    function validateHeader(ValidateHeaderArgs memory _args) internal {
        require(
            _args.header.coinbase != address(0),
            Errors.Rollup__InvalidCoinbase()
        );
        require(
        ...
        require(
            ...
            Errors.Rollup__TimestampInFuture(currentTime, timestamp)
        );
        ...
        require(
            _args.header.gasFees.feePerDaGas == 0,
    ...
    function digest(
        ProposePayload memory _args
    ) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(SignatureDomainSeparator.blockAttestation, _args)
            );
    }
}
