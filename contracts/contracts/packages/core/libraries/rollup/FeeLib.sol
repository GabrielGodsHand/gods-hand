...
library FeeLib {
    ...
    function getManaBaseFeeComponentsAt(
...

        {
            ...
            // Sequencer cost per mana
            {
                ...
                sequencerCostPerMana = EthValue.wrap(
                    Math.mulDiv(ethUsed, 1, manaTarget, Math.Rounding.Ceil)
                );
            }
        ...
        return
            ManaBaseFeeComponents({
                sequencerCost: FeeAssetValue.unwrap(
                    sequencerCostPerMana.toFeeAsset(feeAssetPrice)
                ),
                proverCost: FeeAssetValue.unwrap(
                    proverCostPerMana.toFeeAsset(feeAssetPrice)
                ),
    ...
    function summedBaseFee(
        ...
        return
            _components.sequencerCost +
            _components.proverCost +
