...
library BlobLib {
        ...
        )
    {
        ...
        for (uint256 i = 0; i < numBlobs; i++) {
            ...
            blobCommitments[i] = abi.encodePacked(
                _blobsInput[blobInputStart:blobInputStart +
            ...
            if (_checkBlob) {
                ...
                require(
                    ...
                    Errors.Rollup__InvalidBlobHash(blobHash, blobHashCheck)
                );
            } else {
                blobHash = blobHashCheck;
            }
    ...
    function validateBatchedBlob(
        ...
        require(
            success,
            Errors.Rollup__InvalidBlobProof(bytes32(_blobInput[0:32]))
        );
    }
    ...
    function calculateBlobCommitmentsHash(
        ...
        if (_isFirstBlockOfEpoch) {
            ...
            currentblobCommitmentsHash = Hash.sha256ToField(
                abi.encodePacked(_blobCommitments[i++])
            );
        }
