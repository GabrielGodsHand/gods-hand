...
import {ValidatorSelectionStorage} from "../../interfaces/IValidatorSelection.sol";
...
library ValidatorSelectionLib {
    ...
    string private constant PROPOSER_NAMESPACE =
        "aztec.validator_selection.transient.proposer";
    ...
    function setupEpoch(Epoch _epochNumber) internal {
        ...
        if (committeeCommitment == bytes32(0)) {
            address[] memory committee = sampleValidators(
                _epochNumber,
                sampleSeed
            );
            store.committeeCommitments[
                _epochNumber
            ] = computeCommitteeCommitment(committee);
        }
    }
    ...
    function verify(
        ...
        address[] memory reconstructedCommittee = new address[](
            targetCommitteeSize
        );
        ...
        bytes32 reconstructedCommitment = computeCommitteeCommitment(
            reconstructedCommittee
        );
    ...
    function setCachedProposer(Slot _slot, address _proposer) internal {
        PROPOSER_NAMESPACE
            .erc7201Slot()
            .deriveMapping(Slot.unwrap(_slot))
            .asAddress()
            .tstore(_proposer);
    }
    ...
    function sampleValidators(
        ...
        require(
            ...
            Errors.ValidatorSelection__InsufficientCommitteeSize(
                validatorSetSize,
                targetCommitteeSize
            )
        );
        ...
        return
            StakingLib.getAttestersFromIndicesAtTime(
                Timestamp.wrap(ts),
                indices
            );
    }
    ...
    function getCommitteeCommitmentAt(
        Epoch _epochNumber
    ) internal returns (bytes32 committeeCommitment, uint256 committeeSize) {
        ValidatorSelectionStorage storage store = getStorage();
        ...
        if (committeeCommitment == 0) {
            ...
            committeeCommitment = computeCommitteeCommitment(
                sampleValidators(_epochNumber, getSampleSeed(_epochNumber))
            );
        }
    ...
    function epochToSampleTime(Epoch _epoch) internal view returns (uint32) {
        ...
        return
            ...
            uint32(TimeLib.getEpochDurationInSeconds()) -
            1;
    }
    ...
    function computeNextSeed(Epoch _epoch) private view returns (uint224) {
        ...
        return
            uint224(uint256(keccak256(abi.encode(_epoch, block.prevrandao))));
    }
     ...
     */
    function computeCommitteeCommitment(
