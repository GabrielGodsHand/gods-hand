...
import {RewardConfig, ActivityScore} from "../libraries/rollup/RewardLib.sol";
import {IHaveVersion} from "../../governance/interfaces/IRegistry.sol";
...
interface IRollup is IRollupCore, IHaveVersion {
    ...
    ) external view returns (bytes32[] memory, bytes32, bytes[] memory);
    ...
    function getActivityScore(
        address _prover
    ) external view returns (ActivityScore memory);
