...
contract FeeJuicePortal is IFeeJuicePortal {
    ...
    bytes32 public constant L2_TOKEN_ADDRESS =
        bytes32(Constants.FEE_JUICE_ADDRESS);
    ...
    function depositToAztecPublic(
        ...
        uint256 _amount,
        bytes32 _secretHash
    ) external override(IFeeJuicePortal) returns (bytes32, uint256) {
        // Preamble
        ...
        bytes32 contentHash = Hash.sha256ToField(
            abi.encodeWithSignature("claim(bytes32,uint256)", _to, _amount)
        );
    ...
    function distributeFees(
        ...
        require(
            ...
            Errors.FeeJuicePortal__Unauthorized()
        );
