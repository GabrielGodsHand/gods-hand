...
import {FrontierLib} from "../libraries/crypto/FrontierLib.sol";
import {Hash} from "../libraries/crypto/Hash.sol";
import {DataStructures} from "../libraries/DataStructures.sol";
import {Errors} from "../libraries/Errors.sol";
import {FeeJuicePortal} from "./FeeJuicePortal.sol";
...
contract Inbox is IInbox {
    ...
    constructor(
        ...
        EMPTY_ROOT = trees[uint64(Constants.INITIAL_L2_BLOCK_NUM) + 1].root(
            forest,
            HEIGHT,
            SIZE
        );

        FEE_ASSET_PORTAL = address(
            new FeeJuicePortal(
                IRollup(_rollup),
                _feeAsset,
                IInbox(this),
    ...
    function sendL2Message(
        ...
        require(
            _recipient.version == VERSION,
            Errors.Inbox__VersionMismatch(_recipient.version, VERSION)
        );
        require(
            uint256(_content) <= Constants.MAX_FIELD_VALUE,
            Errors.Inbox__ContentTooLarge(_content)
        );
        require(
        ...
        uint256 index = (inProgress - Constants.INITIAL_L2_BLOCK_NUM) *
            SIZE +
            currentTree.nextIndex;
        ...
        address senderAddress = msg.sender == FEE_ASSET_PORTAL
            ? address(uint160(Constants.FEE_JUICE_ADDRESS))
            : msg.sender;
        ...
        bytes16 updatedRollingHash = bytes16(
            keccak256(abi.encodePacked(rollingHash, leaf))
        );
        state = InboxState({
    ...
    function consume(
        ...
        require(
            _toConsume < inProgress,
            Errors.Inbox__MustBuildBeforeConsume()
        );
    ...
    ) external view override(IInbox) returns (bytes32) {
    ...
    function getState()
        ...
        view
        override(IInbox)
        returns (InboxState memory)
    {
        return state;
    }

    function getTotalMessagesInserted()
        external
        view
        override(IInbox)
        returns (uint64)
