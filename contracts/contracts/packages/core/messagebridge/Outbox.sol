...
import {MerkleLib} from "../libraries/crypto/MerkleLib.sol";
...
contract Outbox is IOutbox {
    ...
    function consume(
        ...
        require(
            _l2BlockNumber <= ROLLUP.getProvenBlockNumber(),
            Errors.Outbox__BlockNotProven(_l2BlockNumber)
        );
        require(
        ...
        require(
            ...
            Errors.Outbox__InvalidRecipient(
        ...
        require(
            block.chainid == _message.recipient.chainId,
            Errors.Outbox__InvalidChainId()
        );
        ...
        require(
            blockRoot != bytes32(0),
            Errors.Outbox__NothingToConsumeAtBlock(_l2BlockNumber)
        );
    ...
    function hasMessageBeenConsumedAtBlockAndIndex(
        uint256 _l2BlockNumber,
        uint256 _leafIndex
    ) external view override(IOutbox) returns (bool) {
    ...
    function getRootData(
        uint256 _l2BlockNumber
    ) external view override(IOutbox) returns (bytes32) {
