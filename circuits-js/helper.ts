...
export class ZKPassportHelper {
  ...
  private static validateProofData(
    ...
    if (vkey.length !== this.VKEY_SIZE) {
      throw new Error(
        `Invalid vkey size for circuit ${circuitType}: expected ${this.VKEY_SIZE}, got ${vkey.length}`
      );
    }
    if (formattedProofData.length !== this.PROOF_SIZE) {
      throw new Error(
        `Invalid proof size for circuit ${circuitType}: expected ${this.PROOF_SIZE}, got ${formattedProofData.length}`
      );
    }
    if (![2, 5, 10].includes(publicInputs.length)) {
      throw new Error(
        `Invalid public inputs size for circuit ${circuitType}: expected 2, 5, or 10, got ${publicInputs.length}`
      );
    }
  ...
  private static findProofsByKeywords(proofs: ProofResult[]): {
    ...
    return {
      proofA: proofs.find((p) =>
        p.name?.toLowerCase().includes(proofKeywords.A)
      ),
      proofB: proofs.find((p) =>
        p.name?.toLowerCase().includes(proofKeywords.B)
      ),
      proofC: proofs.find((p) =>
        p.name?.toLowerCase().includes(proofKeywords.C)
      ),
      proofD: proofs.find((p) =>
        p.name?.toLowerCase().includes(proofKeywords.D)
      ),
    };
  }
  ...
  public static async formatProofsForContract(
    ...
    try {
      ...
      if (proofs.length !== 4) {
        console.error(
          `Incorrect number of proofs: expected 4, got ${proofs.length}`
        );
        return undefined;
      }
      ...
      const { proofA, proofB, proofC, proofD } =
      ...
      return this.createContractProofData(
        formattedProofs,
        chainValidation.scopedNullifier
      );
    } catch (error) {
